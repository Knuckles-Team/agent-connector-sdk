#!/usr/bin/env python3
# Copied from agent-packages/agent-utilities/scripts/check_import_cycles.py for agent-connector-sdk (RF-ADR-009 lane SDK-CORE). Adapted: default package agent_connector_sdk/.
...
def _parse_args(argv: list[str] | None) -> Path:
    ...
    ap.add_argument(
        ...
        nargs="?",
        default=None,
        help="Path to the package directory to scan (default: agent_connector_sdk).",
    )
    ns = ap.parse_args(argv)
    if ns.package_root:
        return Path(ns.package_root).resolve()
    return _AU_ROOT / "agent_connector_sdk"
