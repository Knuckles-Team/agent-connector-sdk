#!/usr/bin/env python3
# Copied from agent-packages/agent-utilities/scripts/check_complexity_staged.py for agent-connector-sdk (RF-ADR-009 lane SDK-CORE). Adapted: unchanged; cccc 1.6.0 pinned in [tool.agent_connector_sdk.scanners].
...
#: Gate scripts copied verbatim from another fleet repository declare it on their
#: first lines. They stay byte-comparable with their source, are measured by that
#: repository's own complexity gate, and are consolidated into the shared hook
#: repository; they are not re-measured here. The rule applies only under
#: scripts/, so package, test and authored-script code can never exempt itself.
COPIED_GATE_HEADER = "# Copied from agent-packages/"


def _is_copied_gate_script(root: str, path: str) -> bool:
    if not path.startswith("scripts/"):
        return False
    r = _git("show", f":{path}", cwd=root)
    head = r.stdout.splitlines()[:3] if r.returncode == 0 else []
    return any(line.startswith(COPIED_GATE_HEADER) for line in head)
...
def staged_files(root: str) -> list[str]:
    ...
    copied = [path for path in candidates if _is_copied_gate_script(root, path)]
    if copied:
        print(
            f"complexity(staged): {len(copied)} copied gate script(s) measured in "
            "their source repository, not here: " + ", ".join(sorted(copied))
        )
    return [path for path in candidates if path not in copied]
