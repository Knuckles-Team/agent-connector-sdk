#!/usr/bin/env python3
# Copied from agent-packages/agent-utilities/scripts/check_event_loop_blocking.py for agent-connector-sdk (RF-ADR-009 lane SDK-CORE). Adapted: scans agent_connector_sdk/.
...
def _changed_py_files(root: str) -> list[str]:
    ...
    for args in (
        ...
        r = _git(*args, "--", "agent_connector_sdk", cwd=root)
        if r.returncode == 0:
