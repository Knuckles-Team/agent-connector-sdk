# Copied from agent-packages/agent-utilities/scripts/check_stubs.py for agent-connector-sdk (RF-ADR-009 lane SDK-CORE). Adapted: see DECLARED_CONTRACT_SEAMS.
import argparse
import ast
import io
import os
...
#: epistemic-graph has not published yet (RF-ADR-009 W1). A function in the named
#: file whose body raises NotImplementedError with one of the named message
#: constants is reported as NOT DONE instead of failing; every other stub, in that
#: file or any other, still fails. Delete an entry when the owning wave lands.
DECLARED_CONTRACT_SEAMS = {
    "agent_connector_sdk/sinks/epistemic_graph.py": frozenset(
        {"PACK_IMPORT_UNAVAILABLE", "RECORD_INGESTION_UNAVAILABLE"}
    ),
}


def _declared_seam_constants(filepath):
    normalized = str(filepath).replace(os.sep, "/")
    for suffix, constants in DECLARED_CONTRACT_SEAMS.items():
        if normalized == suffix or normalized.endswith("/" + suffix):
            return constants
    return frozenset()


def _raises_declared_seam(node, constants):
    """True when the body is a docstring plus ``raise NotImplementedError(CONST)``."""
    statements = [
        s
        for s in node.body
    ...
    return (
        ...
        and exc.args[0].id in constants
    )
    ...
    try:
        ...
        class StubVisitor(ast.NodeVisitor):
            ...
            def _handle_function(self, node):
                ...
                seam_constants = _declared_seam_constants(filepath)
                if seam_constants and _raises_declared_seam(node, seam_constants):
                    print(
                        f"NOT DONE (declared contract seam): {filepath}:{node.lineno}: {node.name}"
                    )
                    return
                if (
                    is_abstract
