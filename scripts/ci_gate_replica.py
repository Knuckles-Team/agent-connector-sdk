#!/usr/bin/env python3
# Copied from agent-packages/agent-utilities/scripts/ci_gate_replica.py for agent-connector-sdk (RF-ADR-009 lane SDK-CORE). Adapted: WORKFLOW_REGISTRY and BUILD_AFFECTING_FILE_PATTERNS describe this repository's workflows.
...
WORKFLOW_REGISTRY: dict[str, WorkflowSpec] = {
    "release.yml": WorkflowSpec(
        ...
        job_skip_reasons={
            "scanner-quality": (
                "provisions the pinned cccc/kiss/dupehound/jscpd toolchain on the runner "
                "(cargo install, npm install) and runs the diff and census gates over an "
                "immutable commit range; local replay never installs tools, and the "
                "pre-commit hooks run the same wrappers against the locally installed, "
                "version-verified binaries."
            ),
            "publish-pypi": (
                "requires a vX.Y.Z release tag and PyPI trusted publishing; publishing "
                "must never happen from a local hook."
            ),
        },
    ),
    "pages.yml": WorkflowSpec(
        filename="pages.yml",
        blocking=True,
        executable_jobs=frozenset({"build"}),
        job_skip_reasons={
            "deploy": (
                "deploys to the GitHub Pages environment, which has no local equivalent; "
                "the build job it depends on is replayed."
            ),
        },
    ),
}
