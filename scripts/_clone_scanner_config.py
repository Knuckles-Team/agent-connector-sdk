# Copied from agent-packages/agent-utilities/scripts/_clone_scanner_config.py for agent-connector-sdk (RF-ADR-009 lane SDK-CORE). Adapted: reads [tool.agent_connector_sdk.clone_scanners].
...
def _require(mapping: Mapping[str, Any], name: str) -> Any:
    try:
        ...
        raise CloneScannerConfigError(
            f"[tool.agent_connector_sdk.clone_scanners] is missing {name!r}"
        ) from exc
...
def load_clone_scanner_config(path: Path) -> CloneScannerConfig:
    ...
    try:
        table = document["tool"]["agent_connector_sdk"]["clone_scanners"]
    except (KeyError, TypeError) as exc:
        raise CloneScannerConfigError(
            "pyproject.toml has no [tool.agent_connector_sdk.clone_scanners] table"
        ) from exc
    if not isinstance(table, Mapping):
        raise CloneScannerConfigError(
            "[tool.agent_connector_sdk.clone_scanners] must be a TOML table"
        )
