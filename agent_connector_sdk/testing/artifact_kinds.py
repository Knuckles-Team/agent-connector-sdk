...
def _mapping_problems(
    kind: ArtifactKind, entries: Sequence[ArtifactEntry]
) -> list[str]:
    problems: list[str] = []
    for entry in entries:
        try:
            record = kind.to_record(entry)
        except MalformedArtifactError as exc:
            problems.append(f"{entry.uri}: {exc}")
            continue
        if record.entry_digest != entry.digest or record.uri != entry.uri:
            problems.append(f"{entry.uri}: record does not reference its entry")
    return problems
...
async def check_artifact_kind(
    ...
    problems = _mapping_problems(kind, first)
    stable = [entry.digest for entry in first] == [entry.digest for entry in second]
    return [
        ConformanceResult(
...
def check_artifact_rejects_malformed(
    kind: ArtifactKind, malformed: ArtifactEntry
) -> ConformanceResult:
